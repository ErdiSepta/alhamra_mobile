...
class StudentService {
  ...
  Future<List<StudentModel>> getStudentsByParent(int orangtuaId) async {
    if (_useOdooApi) {
      try {
        print('StudentService: Fetching students for orangtua_id: $orangtuaId');
        
        // Get from Odoo API
        final studentsData = await _odooApi.getStudents(orangtuaId);
        
        print('StudentService: Received ${studentsData.length} students from Odoo');
        
        // Convert to StudentModel list
        final students = studentsData
            .map((data) => StudentModel.fromOdoo(data))
            .toList();
