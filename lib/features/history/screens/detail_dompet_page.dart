                    ...
                    children: [
                      ...
                      Column(
                        children: [
                          ...
                          const SizedBox(height: 12),
                          _buildDetailRow('Waktu', DateFormat('dd MMMM yyyy, HH:mm').format(transaction.date) + ' WIB'),
                          const SizedBox(height: 12),
                          _buildDetailRow('Bank', transaction.bankName),
                          if (transaction.referenceId != null && transaction.referenceId!.isNotEmpty) ...[
                            const SizedBox(height: 12),
                            _buildDetailRow('Referensi', transaction.referenceId!),
                          ],
                      ...
                      Column(
                        children: [
                          _buildDetailRow('Nominal', (transaction.isPositive ? '+' : '-') + _formatCurrency(transaction.amount)),
                          const SizedBox(height: 12),
...
• Nominal: ${(transaction.isPositive ? '+' : '-')}${_formatCurrency(transaction.amount)}
• Waktu: ${DateFormat('dd MMMM yyyy, HH:mm').format(transaction.date)} WIB
• Bank: ${transaction.bankName}${transaction.referenceId != null ? '\n• Referensi: ${transaction.referenceId}' : ''}${transaction.description != null ? '\n• Catatan: ${transaction.description}' : ''}
